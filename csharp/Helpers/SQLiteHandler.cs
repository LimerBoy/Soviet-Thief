...
namespace YandexDecryptor.Helpers
{
    public class SQLiteHandler
    {
        ...
        {
            this.db_bytes = IOUtils.ReadFileBytes(baseName);
        ...
        public byte[] GetBytes(int row_num, int field)
        {
            string content = GetValue(row_num, field);
            return Encoding.Default.GetBytes(content);
        }

        public byte[] GetBytes(int row_num, string field)
        {
            string content = GetValue(row_num, field);  
            return Encoding.Default.GetBytes(content);
        }
